...
-   Kolumndefinitionens huvudområde innehåller den rubriktext och formatering som visas i rapporten. En rubrik kan gälla för en kolumn med data kan sträcka sig över flera kolumner eller kan användas på basis av villkorlig. Kolumndefinitionen kan innehålla så många kolumner rubrikrader som du behöver. **Obs!** Kolumnrubriker gäller för varje datakolumn i rapporten. Rapportrubriker gäller för hela rapporten. Du definierar rapportrubriker på fliken **Sidhuvud och sidfot** i rapportdefinitionen.
...
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SU                      | Ignorera understrecket för en kolumn om en understrykningskommando (**---**) eller dubbelunderstrykningskommando (**===**) anges i raddefinitionen. Du kan till exempel välja att inte understryka belopp som produceras av en procentsatsberäkning.                                                                        |
...
Rapportutformaren kan skapa kolumnrubriker automatiskt baserat på autotextkoder. Autotextkoder är variabler som uppdateras varje gång som en rapport genereras. Alla kolumnrubriker kan innehålla dessa koder för att ange rapportinformation som kan variera, t.ex. datum eller periodnummer. Därför kan du använda en kolumndefinition för flera rapportdefinitioner, tidsperioder och rapportträd. Eftersom autotextkoder hänvisar till kalenderinformationen från detaljraderna i kolumndefinitionen, stöds de endast för kolumnerna **CALC**, **FD** och **WKS**. Sättet som en autotextkod visas i kolumnrubriken inverkar på hur den informationen visas i rapporten. I dialogrutan **Kolumnrubrik** visas autotextkoderna när gemener och versaler blandas. Därför visas texten i rapporten när gemener och versaler blandas. Till exempel i ett Standardkalenderår **@CalMonthLong**månad matchar **7** till **juli**. Om månadens namn ska visas med versaler (till exempel **juli**), ange autotextkod med versaler i den **rubriktext** fält. Till exempel anger**@CALMONTHLONG**. Du kan blanda koder och text. Exempelvis anger du följande rubriktext: **Period @FiscalPeriod-@FiscalYearfrån @StartDatetill@EndDate**. Rapportrubriken som skapas liknar följande text: **Period 1-02 från 01/01/02 till 01/31/02**. **Anmärkning:** formatet för text, till exempel långt datumformat beror på de nationella inställningarna för Dynamics 365 för servern operationer. För att ändra dessa inställningar, klicka på knappen **Starta**, klicka på **Kontrollpanelen** och klicka sedan på **Region och språk**. Följande tabeller listar de tillgängliga autotextalternativen för kolumnrubriker.
...
| Förkortning för månad (@CalMonthShort) | Skriv ut det förkortade månadsnamnet för den valda räkenskapsperioden.                                                                                                                                                                                                                          |
...
| Enhetsnamn från träd (@UnitName)         | Om du begränsar en kolumn till en viss enhet av rapportträdet, skriver du ut enhetsnamnet i kolumnrubriken.                                                                                                                                                                                     |
...
| Boka Code (@BookCode)                   | Skriv ut bokkoden som anges i kolumnen.                                                                                                                                                                                                                                             |
...
-   Om du anger kolumnrubriker som överlappar varandra, kommer de att överlappas när de skrivs ut i rapporten. Rapporten genereras och visas följande varning i den **Rapportköstatus** fält: "kolumnrubriker använder Base överlappar andra kolumnrubriker och medföra överlappande text." Exempelvis är rubriken definitionen i kolumn B B BASE + 1 och definitionen rubriken för kolumn D är BASE + 1 till F. I det här fallet rubrikerna skrivs ut ovanpå varandra och är oläsliga. Så snart BAS används i definitionen **Fördela från/fördela till** ska du se till att visa rapporten som skapas, för att se om rubrikerna överlappas.
...
-   För villkorliga utskriftskolumner (**P&lt;B**, **P&gt;= B**), villkorsstyrd rubrikomfång som alla vanliga kolumnrubrikdefinitioner. Om till exempel villkoret är falskt, startar en efterföljande kolumn som matchar det fördelade villkoret en utskrift av rubriken.
...
3.  Välj en kolumntyp i listan. Följande tabell beskriver olika kolumntyper.
    ...
    <tr class="odd">
    <td>ATTR</td>
    <td>Om ditt redovisningssystem stöder attribut visas ett konto- eller transaktionsattribut i kolumnen. Ett attribut som ska användas för ett enkelt fulständigt konto, hämtar underliggande konto eller transaktionsinformation från ekonomiska data. Attributen på kontonivå visar data från kontot och transaktionsnivå-visa data som uppstått vid den tidpunkt då transaktionen bokfördes. Om du väljer <strong>ATTR</strong> som kolumntyp, ange kategorin attribut i den <strong>redovisningskod/Attributkategori</strong> detaljrad kolumndefinitionen.</td>
    </tr>
    </tbody>
    </table>
...
| BASE +\# | Använd året \#år efter basår. Om du till exempel vill använda det tredje året efter basåret, anger du **BASE+3**. |
| BASE-\# | Använd året \#år före basår. Om du till exempel vill använda föregående år, ange **BASE-1**.                 |
...
| BASE +\#         | Använd den period som \#perioder efter basperioden. Om du till exempel vill använda den tredje perioden efter basperioden anger du **BASE+3**.                                                                                               |
| BASE-\#         | Använd den period som \#perioder före basperioden. Om du till exempel vill använda föregående period, ange **BASE-1**.                                                                                                                 |
| BASE -\#: BAS    | Använd flera perioder, från flera perioder före basperioden genom basperioden. Om du till exempel vill använda de tre föregående perioderna och basperioden, anger du **BASE-3:BASE**                                                |
...
Du kan gå bakom räkenskapsårets begränsningar i någon av periodspecifikationerna, och du kan blanda år i ett periodintervall. Till exempel anger du perioderna som **5-** (tidigare motsvarar sex perioder) och kör en rapport med en basperiod 2. I det här fallet visar rapporten data för de första två perioderna i angivet räkenskapsår och sista fyra perioderna i föregående räkenskapsår.
...
3.  Fyll i formeln i formelfältet ovanför navigeringsfönstret eller i cellen **Period**. Ersätt alla nummertecken (\#) med rätt värde.
...
Ett dimensionsfilter används för att begränsa **FD**-kolumnen till specifika dimensionsvärden. Filtret kan innehålla en enda dimension, ett intervall av dimensioner eller en grupp av dimensioner. Filtret kan också inkludera dimensionsvärdeuppsättningar. Eftersom dimensionsvärden kan variera en... \financial-dimensions\dimension-based systemet behöver inte överensstämma med en exakt längd. Filtret används oavsett om rapporten omfattar ett rapportträd. Du kan använda jokertecken (\* eller?) i något läge. Om du anger flera konton anger ett komma mellan konton, som i följande exempel: + Account =\[1200\], + Account =\[1100\], avdelning =\[01?\] Om du vill ta emot alla avdelningar för ett visst konto kan du utelämna avdelningsdimensionen från dimensionfiltret. Om till exempel båda av följande dimensionfilter hanteras på samma sätt:

-   + Konto =\[1100\], avdelning
-   + Konto =\[1100\]

Du kan också använda någon kombination av alfanumeriska tecken för exakt matchning och du kan definiera deldimensioner. Till exempel **plats = \[10\*\]** inkluderar alla platsdimensionsvärden som börjar med 10.
...
-   **Kontots valutavisning** – ange valuta (naturlig funktionella eller reporting) som visas i transaktionerna. Den här funktionen kallas ibland valutaomräkning. Valutaomräkning är förmågan att rapportera redovisningsbelopp i en valuta som inte kanske är företagets funktionella valuta eller den valuta som transaktionen angavs i.
...
**Obs!** Alternativen som beskrivs här kan komma att skilja sig beroende på ERP-systemet. Mer information finns i [dokumentation Microsoft ERP](https://www.microsoft.com/en-us/download/details.aspx?id=5916).
...
| Funktionell valuta från företagsinformation | Tom                | **$2,310\*\*** – resultatet visar alla data i den funktionella valutan som har angetts i företagsinformationen. **Obs!** Detta belopp är summan av alla transaktioner i funktionell valuta. |
...
Du kan skapa en kolumn som visar alla värden i en viss kolumn som en procentandel av ett basnummer. Därför kan du visa relationer mellan rader som till exempel en procentandel av en försäljningrad eller en procentandel av en rad för totala utgifter. Ange om du vill multiplicera eller dividera varje rad i en viss kolumn efter en basrad kolumnen om du vill använda i beräkningen och ange **\*BASEROW** eller **/BASEROW**. Till exempel anger **C\*BASEROW** eller **C/BASRAD**. ** Obs!** Kontrollera att varje raddefinition som används med denna kolumndefinition när du använder en basradberäkning i en kolumndefinition, innehåller minst en basrad för beräkningar.
