...
- Förstå hur funktionen uppgiftsinspelare i Microsoft Dynamics 365 for Finance and Operations tillsammans med Microsoft Dynamics med Lifecycle Services (LCS) och Microsoft Azure DevOps, gör det möjligt att skapa, konfigurera, köra, undersöka och rapportera om testfall.
- Ge funktionella användare möjlighet att registrera affärsuppgifter med hjälp av uppgiftsregistrering i Finance and Operations och sedan konvertera inspelningsuppgifterna till en uppsättning automatiserade tester utan att källkoden behöver skrivas.

## <a name="before-you-begin"></a>Innan du börjar

### <a name="prerequisites"></a>Förutsättningar

- En Finance and Operations-miljö som kör Microsoft Dynamics 365 for Finance and Operations version 10.0 (april 2019) eller senare krävs för den här självstudien. För kunder som använder Microsoft Dynamics 365 for Finance and Operations Enterprise Edition 7.3, plattformsuppdatering 20 (PU20) eller senare stöds också.
- Användaren måste ha administratörsrättigheter till Finance and Operations-miljö.
...
## <a name="finance-and-operations-environment"></a>Finance and Operations-miljön

### <a name="version-requirement"></a>Versionskrav

Ett test- eller begränsat läge-miljö som kör version 10 krävs för ett Finance and Operations-test. För kunder som använder version 7.3, plattformsuppdatering 20 eller senare stöds också.

> [!NOTE]
> RSAT måste ha åtkomst till Finance and Operations-testmiljön via en webbläsare.
...
Det här steget krävs för att ansluta med LCS så att uppgiftsregistreringar kan sparas i lämpligt BPM-bibliotek i LCS via Finance and Operations-klienten.

1. Öppna Finance and Operations-klienten
...
> [!NOTE]
> Kontrollera att alla uppgiftsinspelningar startar på huvudinstrumentpanelen i Finance and Operations. Hålla enskilda inspelningar korta och fokusera på en affärsuppgift, t.ex. skapa en försäljningsorder.
...
RSAT kan installeras på alla datorer som kör Windows 10 och som kan ansluta till Finance and Operations-miljö via en webbläsare (Internet Explorer eller Google Chrome).
...
> [!NOTE]
> Om Finance and Operations-miljön är en Nivå 2+-miljö, måste tumavtrycket för certifikatet uppdateras i filen wif.config för **alla** AOS-datorer för miljön.
...
4. Uppdatera filen **wif.config** genom att lägga till en ny auktoritetspost för ditt certifikat och auktoritetsnamn, som i följande exempel.
    ...
    > [!NOTE]
    > Om flera användare använder samma Finance and Operations-program, måste varje användare generera separata tumavtryck och alla dessa tumavtryck måste läggas till i avsnittet **\<nycklar\>**.
...
Om en nivå 2+ (sandbox med standardacceptanstest eller högre) Finance and Operations-miljö används, kontrollerar eller anger du följande registerinställning på datorn där RSAT har installerats. Det här steget är obligatoriskt eftersom det hjälper dig att undvika anslutningsfel för autentisering eller RSAT.
...
3. Välj knappen **inställningar** (kugghjulssymbolen) i det övre högra hörnet och fyll sedan i följande fält i dialogrutan som visas:
    ...
    - **Värdnamn** – ange värdnamnet på Finance and Operations-testmiljön, t. ex. **\<myaos\>.cloudax.dynamics.com**. Ta inte med prefixet **https://** eller **http://**.
    - **SOAP-värdnamn** – ange SOAP-värdnamnet för Finance and Operations-testmiljön. Vanligt vis är SOAP-värdnamnet detsamma som värdnamnet, men det har ett **soap**-suffix. Här följer ett exempel: **\<myaos\>soap.cloudax.dynamics.com**. Ta inte med prefixet **https://** eller **http://**.
    ...
    - **Användarnamn för administratören** – Ange e-postadressen för en administratörsanvändare i testmiljön Finance and Operations.
    ...
    - **Företagsnamn** – ange vilket företagsnamn som ska användas som standardföretag för Finance and Operations när Excel-parameterfiler skapas. Du kan ändra företaget senare genom att redigera Excel-parameterfilen.
...
3. Om du vill skapa ett unikt produktnummer för varje körning utan att behöva öppna Excel-parameterfilen på nytt och uppdatera produktnumret manuellt varje gång, använder du följande Excel-formel.
    ...
    > [!NOTE]
    > Förutom fliken **allmänt** innehåller Excel-parameterfilen en dataflik för varje Finance and Operations-formulärsida som testfallet besöker.
