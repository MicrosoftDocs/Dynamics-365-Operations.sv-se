...
Installation av funktionen för e-fakturering är processen att skapa den konfiguration som krävs genom RCS-miljön (Regulatory Configuration Services) och publicera den konfigurationen på servern för e-fakturering. Med hjälp av installationsprogrammet kan du skapa de konfigurerbara reglerna som gör det möjligt för E-fakturering att använda ett säkert protokoll via Internet för att kommunicera och utbyta data med en tredje parts enhet via webbtjänster.
...
## <a name="set-up-the-electronic-invoicing-feature"></a>Ställa in funktionen e-fakturering

Installationsprocessen måste utföras i din RCS-miljö. Följ de här stegen om du vill skapa en ny funktion för e-fakturering.
...
Konfigurationer innehåller de ER-formatkonfigurationer som krävs för omvandling och för att skapa de filer som ska bytas ut under kommunikationen med webbtjänster från andra leverantörer. En funktion för e-fakturering kan ha så många ER-filformatkonfigurationer som behövs, baserat på den tekniska specifikation som tillhandahålls av webbtjänstleverantören.

Följ de här stegen om du vill lägga till ER-format till en funktion för e-fakturering.
...
Funktionsinställningar inkapslar reglerna för kommunikation och säkerhet med en tredje parts webbtjänst. En funktion för e-fakturering kan ha så många funktionsinställningar som krävs, baserat på den affärsregel du vill uppnå.

Följ de här stegen om du vill lägga funktionsinställningar till en funktion för e-fakturering.
...
Åtgärder är en fördefinierad lista med operationer som körs i sekventiell ordning. Den här listan representerar fördelningen av steg som krävs för fullständig körning av funktionen för e-fakturering. Åtgärderna kan inkapslas i samma funktionen för e-fakturering, kommunikationen i båda riktningarna: skickar en begäran om mål och tar emot ett svar och tolkar dess innehåll.
...
Du kan skapa variabler och sedan använda dem som indatavärde för en parameter för en viss åtgärd. Du kan också använda dem för att utbyta, mellan E-fakturering och klienten, information som är resultatet av att en viss åtgärd utförs som en del i flödet av överföringar.
...
En miljö för e-fakturering måste vara kopplad till funktionen för e-fakturering och aktiveras för den. Miljöer för e-fakturering måste skapas och publiceras i förväg, genom konfigurationen av globaliseringsfunktionerna i organisationens RCS-konto.

Följ dessa steg för att aktivera en miljö för e-fakturering för funktionen för e-fakturering.
...
Funktionen för e-fakturering kan delas mellan flera organisationer.

- På sidan **Funktioner för e-fakturering** på **Organisationer** väljer du **Dela med** om du vill lägga till den organisation som du vill dela funktionen för e-fakturering med.

Om du vill sluta dela funktionen för e-fakturering med organisationen väljer du **ta bort delning**.

## <a name="versions"></a>Versioner

Versioner hjälper till att styra livscykeln för den elektroniska faktureringsfunktionen genom att hantera dess status. Du kan skapa en ny version av en befintlig funktion för e-fakturering, eller när all konfiguration för funktionen för e-fakturering är klar kan du ändra funktionens status till **Slutför** och sedan **Publicera**.

### <a name="create-a-new-version-of-an-existing-electronic-invoicing-feature"></a>Skapa en ny version av en befintlig funktion för e-fakturering

1. På sidan **Funktioner för e-fakturering** i rutnätet till vänster välj funktion för e-fakturering.
2. På **versioner** välj **Ny** om du vill lägga till en ny version av funktionen för e-fakturering.

### <a name="change-the-status-of-the-electronic-invoicing-feature"></a>Ändra status för den funktionen för e-fakturering

Följ de här stegen om du vill hantera livscykeln för en funktion för e-fakturering.

1. På sidan **Funktioner för e-fakturering** i rutnätet till vänster välj funktion för e-fakturering.
2. På fliken **versioner** välj **Ändra status** och ändrar sedan statusen från **utkast** till **slutförd**.
3. Du uppmanas att bekräfta att du vill slutföra funktionen för e-fakturering och alla dess komponenter. Välj **Ja** om du vill bekräfta åtgärden eller **Nej** om du vill avbryta åtgärden.

    > [!NOTE]
    > När du väljer **Ja** ändras statusen för konfigurationsversioner, som är komponenter i funktionen för e-fakturering automatiskt från **Utkast** till **Slutförd**.

4. Välj **Ändra status** och ändrar sedan statusen från **Slutför** till **Publicera**.
5. Du uppmanas att bekräfta att du vill publicera funktionen för e-fakturering och alla dess komponenter till den globala databasen. Välj **Ja** om du vill bekräfta åtgärden eller **Nej** om du vill avbryta åtgärden.
