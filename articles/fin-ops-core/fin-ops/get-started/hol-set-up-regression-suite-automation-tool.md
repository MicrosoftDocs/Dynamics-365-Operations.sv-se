...
- <span data-ttu-id="621a9-107">Förstå installationen och nödvändiga inställningar som krävs för de olika programmen som stöder RSAT (Regression Suite Automation Tool).</span><span class="sxs-lookup"><span data-stu-id="621a9-107">Understand the installation and prerequisite setup that are required for the various applications that support Regression suite automation tool (RSAT).</span></span>
...
## <a name="configure-azure-devops"></a><span data-ttu-id="621a9-123">Konfigurera Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="621a9-123">Configure Azure DevOps</span></span>
...
<span data-ttu-id="621a9-173">För det LCS-projektet kan du använda ett befintligt implementerings- eller partnerprojekt för kund.</span><span class="sxs-lookup"><span data-stu-id="621a9-173">For your LCS project, you can use an existing customer implementation or partner project.</span></span>
...
8. <span data-ttu-id="621a9-270">Gör så här om du vill spara uppgiftsinspelningen lokalt och sedan överföra den till BPM via LCS:</span><span class="sxs-lookup"><span data-stu-id="621a9-270">If you want to save the task recording locally and then upload it to BPM through LCS, follow these steps:</span></span>
    ...
    3. <span data-ttu-id="621a9-275">Gå till **RSAT** BPM-biblioteket och välj den affärsprocess som du vill spara uppgiftsinspelningen mot.</span><span class="sxs-lookup"><span data-stu-id="621a9-275">Go to the **RSAT** BPM library, and select the business process to save the task recording against.</span></span>
...
## <a name="get-started-with-rsat"></a><span data-ttu-id="621a9-352">Kom igång med RSAT</span><span class="sxs-lookup"><span data-stu-id="621a9-352">Get started with RSAT</span></span>
...
### <a name="load-and-run-test-cases"></a><span data-ttu-id="621a9-418">Läs in och kör testfall</span><span class="sxs-lookup"><span data-stu-id="621a9-418">Load and run test cases</span></span>
...
4. <span data-ttu-id="621a9-431">Om du vill kontrollera att automatiseringsfiler och parameterfilerna sparas i Azure DevOps, gå till Azure DevOps-projektet och välj **Tavlor \> Arbetsuppgifter** och välj sedan testfallet **Skapa en ny produkt**.</span><span class="sxs-lookup"><span data-stu-id="621a9-431">To verify that the automation files and parameter files are saved to Azure DevOps, go to the Azure DevOps project, select **Boards \> Work Items**, and select the **Create a new product** test case.</span></span> <span data-ttu-id="621a9-432">På fliken **bifogade filer** ska du se fyra filer:</span><span class="sxs-lookup"><span data-stu-id="621a9-432">On the **Attachments** tab, you should see four files:</span></span>
    ...
    - <span data-ttu-id="621a9-433">**.cs** – C\# automatiseringsfil</span><span class="sxs-lookup"><span data-stu-id="621a9-433">**.cs** – C\# automation file</span></span>
...
3. <span data-ttu-id="621a9-482">Om du vill skapa ett unikt produktnummer för varje körning utan att behöva öppna Excel-parameterfilen på nytt och uppdatera produktnumret manuellt varje gång, använder du följande Excel-formel.</span><span class="sxs-lookup"><span data-stu-id="621a9-482">To generate a unique product number for each run without having to reopen the Excel parameter file and manually update the product number every time, use the following Excel formula.</span></span>
...
<span data-ttu-id="621a9-496">En nyckelegenskap i RSAT är en kedja av testfall (det vill säga förmågan hos ett test för att överföra värden till andra tester).</span><span class="sxs-lookup"><span data-stu-id="621a9-496">One key feature of RSAT is the chaining of test cases (that is, the capability of a test to pass values to other tests).</span></span> <span data-ttu-id="621a9-497">Testfall körs enligt deras definierade ordning i Azure DevOps-testplan.</span><span class="sxs-lookup"><span data-stu-id="621a9-497">Test cases are run according to their defined order in the Azure DevOps test plan.</span></span> <span data-ttu-id="621a9-498">(Den här ordningen kan också uppdateras i själva testverktyget.) Om du vill överföra variabler från ett testfall till ett annat, är det därför mycket viktigt att testen finns i rätt ordning.</span><span class="sxs-lookup"><span data-stu-id="621a9-498">(This order can also be updated in the test tool itself.) Therefore, if you want to pass variables from one test case to another, it's very important that the tests be in the correct order.</span></span>

<span data-ttu-id="621a9-499">I det här avsnittet skapar du en sparad variabel i det första testfallet, skapar ett andra testfall och skickar den sparade variabeln från det första testfallet till det andra testfallet.</span><span class="sxs-lookup"><span data-stu-id="621a9-499">In this section, you will create a saved variable in the first test case, create a second test case, and pass the saved variable from the first test case to the second test case.</span></span> <span data-ttu-id="621a9-500">Sedan kör du testfallen som ett kedjetestfall i RSAT.</span><span class="sxs-lookup"><span data-stu-id="621a9-500">You will then run the test cases as a chained test case in RSAT.</span></span>

#### <a name="modify-an-existing-task-recording-to-create-a-saved-variable"></a><span data-ttu-id="621a9-501">Ändra en befintlig uppgiftsinspelning för att skapa en sparad variabel</span><span class="sxs-lookup"><span data-stu-id="621a9-501">Modify an existing task recording to create a saved variable</span></span>

1. <span data-ttu-id="621a9-502">Öppna klienten.</span><span class="sxs-lookup"><span data-stu-id="621a9-502">Open the client.</span></span>
2. <span data-ttu-id="621a9-503">Välj knappen **inställningar** (växelsymbolen) och sedan **Uppgiftsinspelare**.</span><span class="sxs-lookup"><span data-stu-id="621a9-503">Select the **Settings** button (the gear symbol), and then select **Task recorder**.</span></span>
3. <span data-ttu-id="621a9-504">Välj **redigera inspelning**.</span><span class="sxs-lookup"><span data-stu-id="621a9-504">Select **Edit Recording**.</span></span>

    ![Knappen Redigera inspelning](./media/setup_rsa_tool_85.png)

4. <span data-ttu-id="621a9-506">Välj **Öppna från Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="621a9-506">Select **Open from Lifecycle Services**.</span></span>

    ![Knappen Öppna från Lifecycle Services](./media/setup_rsa_tool_86.png)

5. <span data-ttu-id="621a9-508">Välj **Lifecycle Services-bibliotek**</span><span class="sxs-lookup"><span data-stu-id="621a9-508">Select **Select the Lifecycle Services library**.</span></span>

    ![Knappen Välj Lifecycle Services-bibliotek](./media/setup_rsa_tool_87.png)

    <span data-ttu-id="621a9-510">BPM-bibliotek läses in från LCS.</span><span class="sxs-lookup"><span data-stu-id="621a9-510">BPM libraries are loaded from LCS.</span></span>

    ![Förloppsindikator](./media/setup_rsa_tool_88.png)

6. <span data-ttu-id="621a9-512">När du har läst in BPM-biblioteken från LCS väljer du **RSAT** BPM-biblioteket och affärsprocessen **Skapa enny produkt** som uppgiftsinspelningen var associerad med.</span><span class="sxs-lookup"><span data-stu-id="621a9-512">After the BPM libraries are loaded from LCS, select the **RSAT** BPM library and the **Create a new product** business process that the task recording was associated with.</span></span> <span data-ttu-id="621a9-513">Välj sedan **OK**.</span><span class="sxs-lookup"><span data-stu-id="621a9-513">Then select **OK**.</span></span>

    ![Välj ett BPM-bibliotek och en affärsprocess](./media/setup_rsa_tool_89.png)

7. <span data-ttu-id="621a9-515">Namnet på lämplig uppgiftsinspelning anges i fältet **inspelningsnamn**.</span><span class="sxs-lookup"><span data-stu-id="621a9-515">The name of the appropriate task recording is entered in the **Recording name** field.</span></span> <span data-ttu-id="621a9-516">Välj **start**.</span><span class="sxs-lookup"><span data-stu-id="621a9-516">Select **Start**.</span></span>

    ![Namn på uppgiftsinspelningen i fältet Inspelningsnamn](./media/setup_rsa_tool_90.png)

8. <span data-ttu-id="621a9-518">Gå till **produktinformationshantering \> produkter** och välj **Ny** om du vill öppna sidan med den ursprungliga uppgiftsinspelningen, **Skapa en produkt** spelades in.</span><span class="sxs-lookup"><span data-stu-id="621a9-518">Go to **Product information management \> Products**, and select **New** to open the page where the original task recording, **Create a product**, was recorded.</span></span>
9. <span data-ttu-id="621a9-519">Välj **infoga steg**.</span><span class="sxs-lookup"><span data-stu-id="621a9-519">Select **Insert step**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="621a9-520">Det nya steget infogas **efter** steget som du valde i fönstret.</span><span class="sxs-lookup"><span data-stu-id="621a9-520">The new step is inserted **after** the step that you selected in the pane.</span></span>

    ![Knappen Infoga steg](./media/setup_rsa_tool_91.png)

10. <span data-ttu-id="621a9-522">Högerklicka i fältet **produktnummer** och välj sedan **uppgiftsinspelning \> Kopia**.</span><span class="sxs-lookup"><span data-stu-id="621a9-522">Right-click the **Product number** field, and then select **Task recorder \> Copy**.</span></span>

    ![Kopiera kommando](./media/setup_rsa_tool_92.png)

11. <span data-ttu-id="621a9-524">Ett nytt steg läggs till i fönstret.</span><span class="sxs-lookup"><span data-stu-id="621a9-524">A new step is added in the pane.</span></span> <span data-ttu-id="621a9-525">Anteckna värdet i fältet **produktnummer** eftersom det senare kommer att bli nödvändigt.</span><span class="sxs-lookup"><span data-stu-id="621a9-525">Make a note of the value in the **Product number** field, because you will need it later.</span></span>

    ![Nya steg har lagts till](./media/setup_rsa_tool_93.png)

12. <span data-ttu-id="621a9-527">Välj **Redigeringen är slutförd**.</span><span class="sxs-lookup"><span data-stu-id="621a9-527">Select **Done editing**.</span></span>
13. <span data-ttu-id="621a9-528">Välj **Spara till Lifecycle Services**och koppla den nya uppgiftsinspelningen till samma BPM-bibliotek och affärsprocess som den ursprungliga uppgiftsinspelningen var kopplad till.</span><span class="sxs-lookup"><span data-stu-id="621a9-528">Select **Save to Lifecycle Services**, and associate the new task recording with the same BPM library and business process that the original task recording was associated with.</span></span> <span data-ttu-id="621a9-529">Mer information finns i avsnittet [skapa en uppgiftsinspelning och spara den i BPM-biblioteket](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="621a9-529">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>
14. <span data-ttu-id="621a9-530">Gå till BPM-biblioteket och välj **Synkronisera testärenden** om du vill skriva över den uppgiftsinspelning som är kopplad till testfallet i Azure DevOps, enligt beskrivningen i avsnittet [testa synkronisering från BPM till Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="621a9-530">Go to the BPM library, and select **Sync testcases** to overwrite the task recording that is attached to the test case in Azure DevOps, as described in the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>
15. <span data-ttu-id="621a9-531">Öppna RSAT och välj **Läs in** om du vill läsa in alla testfall i testpaketet på nytt.</span><span class="sxs-lookup"><span data-stu-id="621a9-531">Open RSAT, and select **Load** to reload all the test cases in the test suite.</span></span> <span data-ttu-id="621a9-532">Du måste generera om automatiserings- och parameterfiler genom att välja testfall och sedan välja **Ny \> Generera testkörnings- och parameterfiler**, enligt beskrivningen i avsnittet [Läs in och kör testfall](#load-and-run-test-cases).</span><span class="sxs-lookup"><span data-stu-id="621a9-532">You must regenerate the automation and parameter files for the appropriate test case by selecting the test case and then selecting **New \> Generate Test Execution and Parameter files**, as described in the [Load and run test cases](#load-and-run-test-cases) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="621a9-533">Om Excel-parametern har lämnats öppen kommer omgenereringen att misslyckas.</span><span class="sxs-lookup"><span data-stu-id="621a9-533">If the Excel parameter file was left open, regeneration will fail.</span></span> <span data-ttu-id="621a9-534">Kontrollera därför att Excel-parametern för testfall är stängd innan du genererar den nya parameterfilen för Excel.</span><span class="sxs-lookup"><span data-stu-id="621a9-534">Therefore, make sure that the Excel parameter file for the test case is closed before you generate the new Excel parameter file.</span></span>

16. <span data-ttu-id="621a9-535">Välj **redigera** om du vill öppna den nya Excel-parametern.</span><span class="sxs-lookup"><span data-stu-id="621a9-535">Select **Edit** to open the new Excel parameter file.</span></span> <span data-ttu-id="621a9-536">Du kommer att se en ny post för **Sparad variabel** på rad 9.</span><span class="sxs-lookup"><span data-stu-id="621a9-536">You will see a new **Saved variable** entry on line 9.</span></span> <span data-ttu-id="621a9-537">Den här variabeln, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** sparas i uppgiftsinspelningens XML-fil och kan användas i efterföljande tester.</span><span class="sxs-lookup"><span data-stu-id="621a9-537">This variable, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, is saved in the task recording's XML file and can be used in subsequent tests.</span></span>

    ![Sparad variabelpost](./media/setup_rsa_tool_94.png)

#### <a name="create-a-new-test-case"></a><span data-ttu-id="621a9-539">Skapa ett nytt testfall</span><span class="sxs-lookup"><span data-stu-id="621a9-539">Create a new test case</span></span>

1. <span data-ttu-id="621a9-540">Gå till **RSAT** BPM-biblioteket.</span><span class="sxs-lookup"><span data-stu-id="621a9-540">Go to the **RSAT** BPM library.</span></span>
2. <span data-ttu-id="621a9-541">Välj processen **Sample Support Business Process** och välj **redigeringsläge** till höger.</span><span class="sxs-lookup"><span data-stu-id="621a9-541">Select the **Sample Support Business Process** process, and then, on the right, select **Edit mode**.</span></span>
3. <span data-ttu-id="621a9-542">Ändra värdet för både fältet **Namn** och fältet **Beskrivning** för att **Frisläpp en produkt**.</span><span class="sxs-lookup"><span data-stu-id="621a9-542">Change the value of both the **Name** field and the **Description** field to **Release a product**.</span></span> <span data-ttu-id="621a9-543">Välj sedan **Spara**.</span><span class="sxs-lookup"><span data-stu-id="621a9-543">Then select **Save**.</span></span>

    ![Namn och beskrivning har ändrats till Frisläpp en produkt](./media/setup_rsa_tool_95.png)

#### <a name="create-a-new-task-recording-that-has-a-validate-function"></a><span data-ttu-id="621a9-545">Skapa en ny uppgiftsinspelning som har en valideringsfunktion</span><span class="sxs-lookup"><span data-stu-id="621a9-545">Create a new task recording that has a Validate function</span></span>

- <span data-ttu-id="621a9-546">Skapa en uppgiftsinspelning om du vill frisläppa den produkt som skapades tidigare till den USRT juridiska personen.</span><span class="sxs-lookup"><span data-stu-id="621a9-546">Create a task recording to release the product that was created earlier to the USRT legal entity.</span></span> <span data-ttu-id="621a9-547">Mer information finns i avsnittet [skapa en uppgiftsinspelning och spara den i BPM-biblioteket](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="621a9-547">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="621a9-548">För sammankopplade testfall rekommenderar vi alltid att du söker efter eller filtrerar posten som du behöver genom att *manuellt skriva värdet i fältet*.</span><span class="sxs-lookup"><span data-stu-id="621a9-548">For chained test cases, we always recommend that you find or filter for the record that you require by *manually typing the value of the field*.</span></span> <span data-ttu-id="621a9-549">På så sätt kan du i verktyget bestämma vilken post som åtgärden måste vidtas mot i det efterföljande testfallet.</span><span class="sxs-lookup"><span data-stu-id="621a9-549">In that way, the tool can determine the record that the action must be taken against in the subsequent test case.</span></span>

    ![Ny uppgiftsinspelning som har en valideringsfunktion](./media/setup_rsa_tool_96.png)

    <span data-ttu-id="621a9-551">Som den föregående visar,efter att produkten hittas genom att använda snabbfiltret, men efter att du har valt **Frisläpp produkter**, måste du validera värdet av fältet **Produktnummer** för att se till att produkt-ID:t är det produkt-ID som skapades tidigare.</span><span class="sxs-lookup"><span data-stu-id="621a9-551">As the preceding illustration shows, after the product is found by using the Quick Filter, but before you select **Release products**, you validate the value of the **Product number** field to make sure that the product ID is the product ID that was created earlier.</span></span> <span data-ttu-id="621a9-552">Om du vill validera värdet högerklickar du på fältet **produktnummer** och väljer sedan **uppgiftsinspelning \> validera \> aktuellt värde**.</span><span class="sxs-lookup"><span data-stu-id="621a9-552">To validate the value, right-click the **Product number** field, and then select **Task recorder \> Validate \> Current Value**.</span></span>

    ![Valider det aktuella värdet](./media/setup_rsa_tool_97.png)

#### <a name="save-the-task-recording-to-bpm"></a><span data-ttu-id="621a9-554">Spara uppgiftsinspelning till BPM</span><span class="sxs-lookup"><span data-stu-id="621a9-554">Save the task recording to BPM</span></span>

1. <span data-ttu-id="621a9-555">När uppgiftsinspelningen har slutförts väljer du **Spara till Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="621a9-555">After the task recording is completed, select **Save to Lifecycle Services**.</span></span>

    ![Alternativ för spara](./media/setup_rsa_tool_98.png)

2. <span data-ttu-id="621a9-557">Biblioteksinformationen läses in från LCS.</span><span class="sxs-lookup"><span data-stu-id="621a9-557">Library information is loaded from LCS.</span></span>

    ![Förloppsindikator](./media/setup_rsa_tool_99.png)

3. <span data-ttu-id="621a9-559">Välj BPM-bibliotek som ska associeras med uppgiftsinspelningen.</span><span class="sxs-lookup"><span data-stu-id="621a9-559">Select the BPM library to associate the task recording with.</span></span> <span data-ttu-id="621a9-560">För den här självstudien väljer du **RSAT** BPM-biblioteket, som skapades tidigare och affärsprocessen **Frisläpp en produkt** under det.</span><span class="sxs-lookup"><span data-stu-id="621a9-560">For this tutorial, select the **RSAT** BPM library that was created earlier and the **Release a product** business process under it.</span></span> <span data-ttu-id="621a9-561">Välj sedan **OK**.</span><span class="sxs-lookup"><span data-stu-id="621a9-561">Then select **OK**.</span></span>

#### <a name="sync-bpm-to-azure-devops"></a><span data-ttu-id="621a9-562">Synkronisera BPM till Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="621a9-562">Sync BPM to Azure DevOps</span></span>

1. <span data-ttu-id="621a9-563">Gå till BPM-biblioteket och öppna biblioteket för **RSAT**.</span><span class="sxs-lookup"><span data-stu-id="621a9-563">Go to the BPM library, and open the **RSAT** library.</span></span>
2. <span data-ttu-id="621a9-564">Välj **VSTS-synkronisering** och **synkronisera testfall**.</span><span class="sxs-lookup"><span data-stu-id="621a9-564">Select **VSTS sync** and then **Sync test cases**.</span></span> <span data-ttu-id="621a9-565">Mer information finns i avsnittet [testa synkroniseringen från BPM till Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="621a9-565">For more information, see the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>

    <span data-ttu-id="621a9-566">När synkroniseringen är slutförd visas den nya arbetsuppgiften och motsvarande testfall för affärsprocessen **frisläppa en produkt** som visas i Azure DevOps på **Tavlor \> Arbetsartiklar**.</span><span class="sxs-lookup"><span data-stu-id="621a9-566">After the synchronization is completed, the new work item and the corresponding test case for the **Release a product** business process appear in Azure DevOps at **Boards \> Work Items**.</span></span>

#### <a name="add-the-new-test-case-to-the-existing-test-suite"></a><span data-ttu-id="621a9-567">Lägg till det nya testfallet i det befintliga testpaketet</span><span class="sxs-lookup"><span data-stu-id="621a9-567">Add the new test case to the existing test suite</span></span>

1. <span data-ttu-id="621a9-568">Gå till **testplaner \> testplaner**och markera planen **RSAT-testplan**.</span><span class="sxs-lookup"><span data-stu-id="621a9-568">Go to **Test plans \> Test plans**, and select the **RSAT Test Plan** plan.</span></span>
2. <span data-ttu-id="621a9-569">Välj **Lägg till befintlig**.</span><span class="sxs-lookup"><span data-stu-id="621a9-569">Select **Add existing**.</span></span>
3. <span data-ttu-id="621a9-570">På sidan **Lägg till testfall i paket** väljer du **Kör fråga**.</span><span class="sxs-lookup"><span data-stu-id="621a9-570">On the **Add test cases to suite** page, select **Run query**.</span></span>
4. <span data-ttu-id="621a9-571">Välj det nya testfallet som har skapats för **Frisläppa en produkt** och välj sedan **Lägg testfall** i det nedre högra hörnet på sidan (den här knappen visas inte i bilden nedan).</span><span class="sxs-lookup"><span data-stu-id="621a9-571">Select the new test case that was created for **Release a product**, and then select **Add test cases** in the lower-right corner of the page (this button isn't shown in the following illustration).</span></span>

    ![Lägg till testfall på sidan Paket](./media/setup_rsa_tool_100.png)

    <span data-ttu-id="621a9-573">Testpaketet har nu två testfall.</span><span class="sxs-lookup"><span data-stu-id="621a9-573">The test suite now has two test cases.</span></span>

    ![Två testfall i testpaketet.](./media/setup_rsa_tool_101.png)

#### <a name="load-test-cases-into-rsat"></a><span data-ttu-id="621a9-575">Läs in testfall i RSAT</span><span class="sxs-lookup"><span data-stu-id="621a9-575">Load test cases into RSAT</span></span>

1. <span data-ttu-id="621a9-576">Öppna RSAT och välj **Läs in**.</span><span class="sxs-lookup"><span data-stu-id="621a9-576">Open RSAT, and select **Load**.</span></span>
2. <span data-ttu-id="621a9-577">Testfallen läses in och du får en varning om att "den här åtgärden skriver över Excel-testdatafilerna, lokala ändringar går förlorade.</span><span class="sxs-lookup"><span data-stu-id="621a9-577">The test cases are loaded, and you receive a warning that states, "This action will overwrite Excel test data files, local changes will be lost.</span></span> <span data-ttu-id="621a9-578">Vill du fortsätta?"</span><span class="sxs-lookup"><span data-stu-id="621a9-578">Do you want to proceed?"</span></span> <span data-ttu-id="621a9-579">Välj **ja** om du vill uppdatera Excel-parametervärdena i det lokala systemet, men inte de Excel-parametervärden som överförts till Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="621a9-579">Select **Yes** to update the Excel parameter files in the local system but not the Excel parameter files that were uploaded to Azure DevOps.</span></span>

    ![Varningsmeddelande](./media/setup_rsa_tool_102.png)

    <span data-ttu-id="621a9-581">Båda testfallen läses in, tillsammans med Excel-parametern för det första testfallet.</span><span class="sxs-lookup"><span data-stu-id="621a9-581">Both the test cases are loaded, together with the Excel parameter file for the first test case.</span></span> <span data-ttu-id="621a9-582">Eftersom du valde **överför** i den senaste körningen hämtas parametervärdena från Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="621a9-582">Because you selected **Upload** in the last run, the parameter files are pulled from Azure DevOps.</span></span>

    ![Testfall har lästs in](./media/setup_rsa_tool_103.png)

3. <span data-ttu-id="621a9-584">Markera bara det andra testfallet och välj sedan **ny \> generera testkörning och parameterstyrda filer**.</span><span class="sxs-lookup"><span data-stu-id="621a9-584">Select only the second test case, and then select **New \> Generate test execution and parameter files**.</span></span>

#### <a name="edit-the-excel-parameter-file"></a><span data-ttu-id="621a9-585">Redigera Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="621a9-585">Edit the Excel parameter file</span></span>

1. <span data-ttu-id="621a9-586">Markera endast det andra testfallet och välj sedan **redigera** för att öppna motsvarande Excel-parameterfil.</span><span class="sxs-lookup"><span data-stu-id="621a9-586">Select only the second test case, and then select **Edit** to open the corresponding Excel parameter file.</span></span>
2. <span data-ttu-id="621a9-587">Kopiera **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** sparad variabel (se avsnittet [Ändra en befintlig uppgiftsinspelning för att skapa en sparad variabel](#modify-an-existing-task-recording-to-create-a-saved-variable)) från det första testfallet till alla fält där produktnumret används.</span><span class="sxs-lookup"><span data-stu-id="621a9-587">Copy the **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** saved variable (see the [Modify an existing task recording to create a saved variable](#modify-an-existing-task-recording-to-create-a-saved-variable) section) from the first test case into all the fields where the product number is used.</span></span> <span data-ttu-id="621a9-588">I det här fallet kopierar du variabeln till fälten **produktnummer** och **validera produktnummer** på arket **EcoResProductListPage**.</span><span class="sxs-lookup"><span data-stu-id="621a9-588">In this case, you copy the variable into the **Product number** and **Validate Product number** fields on the **EcoResProductListPage** sheet.</span></span>

    ![Fält för produktnummer och validera produktnummer](./media/setup_rsa_tool_104.png)

    > [!NOTE]
    > <span data-ttu-id="621a9-590">Variabler kan bara skickas mellan tester under samma testkörning.</span><span class="sxs-lookup"><span data-stu-id="621a9-590">Variables can be passed between tests only during the same test run.</span></span> <span data-ttu-id="621a9-591">Namnen på variablerna måste matcha exakt.</span><span class="sxs-lookup"><span data-stu-id="621a9-591">The names of the variables must match exactly.</span></span>

3. <span data-ttu-id="621a9-592">Markera **Spara** och stäng sedan Excel-arbetsboken.</span><span class="sxs-lookup"><span data-stu-id="621a9-592">Select **Save**, and then close the Excel workbook.</span></span>
4. <span data-ttu-id="621a9-593">Välj **överför** om du vill spara ändringarna som du gjorde i Excel-parameterfilen.</span><span class="sxs-lookup"><span data-stu-id="621a9-593">Select **Upload** to save the changes that you made to the Excel parameter file.</span></span>

#### <a name="run-the-chained-test-cases"></a><span data-ttu-id="621a9-594">Kör kedjetestfallen</span><span class="sxs-lookup"><span data-stu-id="621a9-594">Run the chained test cases</span></span>

1. <span data-ttu-id="621a9-595">Välj båda testfallen och välj sedan **kör**.</span><span class="sxs-lookup"><span data-stu-id="621a9-595">Select both the test cases, and then select **Run**.</span></span>
2. <span data-ttu-id="621a9-596">Kontrollera att båda testfallen har godkänts.</span><span class="sxs-lookup"><span data-stu-id="621a9-596">Verify that both test cases have passed.</span></span>
