---
title: Utbildningskatalog för Finance and Operations
description: Hitta alla utbildningsalternativ för Finance and Operations.
author: loreleishannonmsft
ms.topic: article
ms.date: 04/17/2020
ms.author: v-lshann
audience: Application user
ms.reviewer: margoc
ms.search.scope: Operations
ms.search.region: Global
ms.search.validFrom: 2019-1-29
ms.dyn365.ops.version: 8.0999999999999996
ms.openlocfilehash: 32b67cfd4c007ddc590cd0ee95255d6f3fbc1d17
ms.sourcegitcommit: e9fadf6f6dafdcefaff8e23eaa3c85f53437db3f
ms.translationtype: HT
ms.contentlocale: sv-SE
ms.lasthandoff: 04/22/2020
ms.locfileid: "3279091"
---
# <a name="learning-catalog-for-finance-and-operations"></a>Utbildningskatalog för Finance and Operations

Hitta rätt onlineutbildning, i personliga workshops och evenemang för din roll som användare av Finance and Operations-program. 

<!-- ![Universal Windows Platform (UWP)](images/platform-uwp.png)  -->  

:::row:::
    :::column:::
<!-- ![Universal Windows Platform (UWP)](images/platform-uwp.png)  -->  

    ### <a name="business-and-technical-decision-makers"></a>Affärs- och tekniska beslutsfattare

    Bestämmer du om ni vill investera i ny teknik? 

    [Utbildningskatalog för affärs- och tekniska beslutsfattare](learning-catalog-bdm.md#)

    [Kom igång](learning-catalog-bdm.md#get-started)

    [Prov](learning-catalog-bdm.md#exam)

        :::column-end:::
    :::column:::

    ### <a name="business-users"></a>Företagsanvändare

    Fick du bara en ny applikation att använda? 

    [Utbildningskatalog för affärsanvändare](learning-catalog-business-user.md)

    [Kom igång](learning-catalog-business-user.md#get-started)

    [Kunskap om huvudplattformen](learning-catalog-business-user.md#core-platform-knowledge)

    [Hantering av underleverantörer](learning-catalog-business-user.md#supply-chain-management)

    [Tillverkning](learning-catalog-business-user.md#manufacturing)

    [Ekonomistyrning](learning-catalog-business-user.md#financial-management)

    [Prov](learning-catalog-business-user.md#exam)

    :::column-end:::
:::row-end:::
:::row:::
    :::column:::

    ### <a name="implementation-project-managers"></a>Projektledare för distribution

    Är du ansvarig för att se till att ditt företags implementering går smidigt?

    [Utbildningskatalog för projektledare för implementering](learning-catalog-project-manager.md)

    [Kom igång](learning-catalog-project-manager.md#get-started)

    [Kunskap om huvudplattformen](learning-catalog-project-manager.md#core-platform-knowledge)

    [Ekonomistyrning](learning-catalog-project-manager.md#financial-management)

    [Tillverkning](learning-catalog-project-manager.md#manufacturing)

    [Hantering av underleverantörer](learning-catalog-project-manager.md#supply-chain-management)

    [Prov](learning-catalog-project-manager.md#exams)

        :::column-end:::
    :::column:::

    ### <a name="administrators"></a>Administratörer

    Behöver du hålla system och data flödande, etablerade och säker dygnet runt?

    [Utbildningskatalog för administratörer](learning-catalog-administrator.md)

    [Kom igång](learning-catalog-administrator.md#get-started)

    [Kunskap om huvudplattformen](learning-catalog-administrator.md#core-platform-knowledge)

    [Core-utveckling](learning-catalog-administrator.md#core-development)

    [Migrering och uppgradering](learning-catalog-administrator.md#migration-and-upgrade)

    [Behandling](learning-catalog-administrator.md#servicing)

    [Testar](learning-catalog-administrator.md#testing)

    [Prov](learning-catalog-administrator.md#exam)

  :::column-end:::
:::row-end:::
:::row:::
    :::column:::

    ### <a name="developers"></a>Utvecklare

    Behöver du skriva kod för att integrera med andra datakällor, utöka kärnsystemfunktionaliteten eller bygga ett komplext program?

    [Utbildningskatalog för utvecklare](learning-catalog-developer.md)

    [Kom igång](learning-catalog-developer.md#get-started)

    [Core-utvecklingskomponenter](learning-catalog-developer.md#core-development-concepts)

    [Behandling](learning-catalog-administrator.md#servicing)

    [Migrering och uppgradering](learning-catalog-administrator.md#migration-and-upgrade)

    [Testar](learning-catalog-administrator.md#testing)
        :::column-end:::
    :::column:::

    ### <a name="functional-consultants"></a>Funktionskonsulter

    Är du en implementeringsexpert för en företagsdomän? 

    [Utbildningskatalog för funktionell applikationskonsult](learning-catalog-functional-consultant.md)

    [Kom igång](learning-catalog-functional-consultant.md#get-started)

    [Kunskap om huvudplattformen](learning-catalog-functional-consultant.md#core-platform-knowledge)

    [Ekonomistyrning](learning-catalog-functional-consultant.md#financial-management)

    [Tillverkning](learning-catalog-functional-consultant.md#manufacturing)

    [Hantering av underleverantörer](learning-catalog-functional-consultant.md#supply-chain-management)

    [Prov](learning-catalog-functional-consultant.md#exams)

    :::column-end:::
:::row-end:::
:::row:::
    :::column:::

    ### <a name="partner-sales-and-marketing"></a>Försäljning och marknadsföring för partners

    Är du ansvarig för att hjälpa dina kunder att köpa rätt lösning? 

    [Utbildningskatalog för Microsoft Partner Sales and Marketing](learning-catalog-partner-sales.md)

    [Kom igång](learning-catalog-partner-sales.md#get-started)

    [Prov](learning-catalog-partner-sales.md#exam)

        :::column-end:::
    :::column:::

    ### <a name="solution-architects"></a>Lösningsarkitekter

    Designar du lösningar som tillgodoser kundernas behov och budgetar?

    [Utbildningskatalog för lösningsarkitekter](learning-catalog-solution-architect.md)

    [Kom igång](learning-catalog-solution-architect.md#get-started)

    [Kunskap om huvudplattformen](learning-catalog-solution-architect.md#core-platform-knowledge)

    [Core-utvecklingskomponenter](learning-catalog-solution-architect.md#core-development-concepts)

    [Migrering och uppgradering](learning-catalog-solution-architect.md#migration-and-upgrade)

    [Behandling](learning-catalog-solution-architect.md#servicing)

    [Testar](learning-catalog-solution-architect.md#testing)

    :::column-end:::
:::row-end:::

