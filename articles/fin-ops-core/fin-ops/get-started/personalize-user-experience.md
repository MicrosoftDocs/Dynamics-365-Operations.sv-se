...
- Anpassningar relaterade till sidanvändning kallas *implicita* anpassningar. Exempelvis håller systemet koll på bredden på rutnätskolumner om du justerar dem, och på det expanderade/komprimerade läget för snabbflikar.
...
- **Snabbflikar** - Vissa sidor har expanderbara avsnitt som kallas *snabbflikar*. Appen lagrar information om snabbflikar som du har utökat och komprimerat. Varje gång du kommer tillbaka till sidan kommer samma snabbflikar att visas eller döljs, utifrån din senaste interaktion med sidan. I vissa fall kan du hjälpa till att förbättra systemet genom att komprimera en snabbflik eftersom appen inte behöver hämta informationen för snabbfliken förrän snabbfliken expanderas. Såsom beskrivs senare i det här avsnittet kan du också ändra ordning på snabbflikarna på en sida.
- **Faktarutor** - Vissa sidor har ett fönster som kallas *Faktaruta*. Den här rutan innehåller skrivskyddad information relaterad till det aktuella ämnet på sidan. Varje avsnitt i faktaruta rutan kallas en *faktaruta*. Du kan dölja eller visa hela faktarutan och du kan också visa eller dölja enskilda faktarutor. Appen lagrar dina inställningar. Sedan utifrån varje gång du kommer tillbaka till sidan kommer faktarutans fönster och de enskilda faktarutorna att återställas baserat på din senaste interaktion med sidan. I vissa fall kan du hjälpa till att förbättra systemet genom att komprimera en faktaruta eftersom appen inte behöver hämta informationen för den faktarutan förrän faktarutan expanderas.
- **Åtgärdsfönster** – Ett *åtgärdsfönstret* visas längst upp på de flesta sidor. Åtgärdsfönstret innehåller knappar för många av de åtgärder som du kan utföra på den aktuella sidan. Knapparna ordnas ofta på flikarna. Du kan öppna hela åtgärdsfönstret och du kan konfigurera den så att den komprimeras som standard. Nästa gång du öppnar sidan kommer appen att återställa det fästa tillståndet för åtgärdsfönstret. Om åtgärdsfönstret är fäst öppen, visar appen även fliken med åtgärder som användes senast.
...
Inställningarna för **Dimensionsvisning** tillämpas på alla sidor och åsidosätter eventuella anpassade inställningar av lagerdimensionsfälten på varje enskild sida.

Därför i det föregående exemplet om du inte vill att kolumnen för batchnummerlagerdimensionen ska visas på en sida, måste du avmarkera dimensionen som en del av alternativet **visa mått** för den sidan.
