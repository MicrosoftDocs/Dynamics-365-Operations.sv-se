...
description: Hur du initierar Dataverse eller andra Dynamics 365-appdata med företagsinformation innan du aktiverar dubbelriktad anslutning.
...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="778c6-103">Vanliga frågor om initiering med företagsdata</span><span class="sxs-lookup"><span data-stu-id="778c6-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [rename-banner](~/includes/cc-data-platform-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="778c6-104">Varför måste jag använda initiering?</span><span class="sxs-lookup"><span data-stu-id="778c6-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="778c6-105">Du kanske har en befintlig Dataverse eller annan Dynamics 365-appinstans med affärsdata och du vill aktivera dubbelriktad anslutning mot den.</span><span class="sxs-lookup"><span data-stu-id="778c6-105">You might have an existing Dataverse or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="778c6-106">I detta fall måste du initiera Dataverse eller andra Dynamics 365-appdata med företagsinformation innan du aktiverar dubbelriktad anslutning.</span><span class="sxs-lookup"><span data-stu-id="778c6-106">In this case, you need to bootstrap Dataverse or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="778c6-107">När ska jag använda initiering?</span><span class="sxs-lookup"><span data-stu-id="778c6-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="778c6-108">Du bör använda initieringsfunktionen för att aktivera dubbelriktade tabellmappningar (under steg #5).</span><span class="sxs-lookup"><span data-stu-id="778c6-108">You should use bootstrapping before enabling dual-write table maps (during step #5).</span></span>  
1. <span data-ttu-id="778c6-109">Om du vill ställa in dubbelriktad anslutning mellan olika instanser av din Finance and Operations-app och Dataverse eller andra Dynamics 365-appar, loggar du in på Finance and Operations-appen som administratör.</span><span class="sxs-lookup"><span data-stu-id="778c6-109">To setup the dual-write connection between instances of your Finance and Operations app and the Dataverse or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="778c6-110">Gå till modulen **datahantering** och klicka på knappen **dubbelriktad**.</span><span class="sxs-lookup"><span data-stu-id="778c6-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="778c6-111">Då startas **dataintegratorn**.</span><span class="sxs-lookup"><span data-stu-id="778c6-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="778c6-112">Skapa dubbelriktad anslutning för ett eller flera företag.</span><span class="sxs-lookup"><span data-stu-id="778c6-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="778c6-113">![Skapa dubbelriktad anslutning](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="778c6-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="778c6-114">Aktivera tabellmappningen **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="778c6-114">Enable the **Cdm_companies** table map.</span></span> <span data-ttu-id="778c6-115">Detta synkroniserar företag från Finance and Operations-appen till Dataverse.</span><span class="sxs-lookup"><span data-stu-id="778c6-115">This synchronizes companies from the Finance and Operations app to Dataverse.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="778c6-116">![Aktivera tabellmappningen](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="778c6-116">![Enable the table map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="778c6-117">Kör exempelinitieringskoden på Dataverse eller andra Dynamics 365-appinstanser.</span><span class="sxs-lookup"><span data-stu-id="778c6-117">Run the sample bootstrapping code on the Dataverse or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="778c6-118">När initieringen är klar och systemet är klart för live-synkronisering aktiverar du tabellmappningarna.</span><span class="sxs-lookup"><span data-stu-id="778c6-118">When the bootstrapping is done and the system is ready for the live sync, enable the table maps.</span></span>  

    <span data-ttu-id="778c6-119">Om du aktiverar tabellmappningarna utlöses den inledande datasynkroniseringen för de aktiverade tabellmappningarna.</span><span class="sxs-lookup"><span data-stu-id="778c6-119">Enabling the table maps triggers the initial data sync for the enabled table maps.</span></span> <span data-ttu-id="778c6-120">De data som motsvarar de företag som valts för dubbelriktad anslutning synkroniseras mellan Finance and Operations-appen och Dataverse.</span><span class="sxs-lookup"><span data-stu-id="778c6-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Dataverse.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="778c6-121">Hur ska jag använda kodexemplet?</span><span class="sxs-lookup"><span data-stu-id="778c6-121">How to I use the code sample?</span></span>
<span data-ttu-id="778c6-122">Exempelkoden är ett C#-program som du kan läsa in i Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="778c6-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="778c6-123">Det krävs NuGet-paketberoenden på Dataverse SDK som du kan uppdatera via Visual Studio standardverktyg.</span><span class="sxs-lookup"><span data-stu-id="778c6-123">It takes NuGet package dependencies on the Dataverse SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="778c6-124">När du har packat upp och öppnat lösningen i Visual Studio och återställt NuGet-paketen söker du efter **TODO** i koden.</span><span class="sxs-lookup"><span data-stu-id="778c6-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="778c6-125">Varje beslut som du måste fatta om hur du vill initiera företagsinformation noteras genom **TODO** med exempelkod för en kanonisk implementering.</span><span class="sxs-lookup"><span data-stu-id="778c6-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="778c6-126">Exempelkoden visar bara ett av många sätt som du kan kategorisera entitetsrader efter företag.</span><span class="sxs-lookup"><span data-stu-id="778c6-126">The sample code shows only one of many ways you might categorize entity rows by company.</span></span> <span data-ttu-id="778c6-127">Genom att ändra logiken i avsnitten **TODO** kan du skapa en anpassad kategorisering.</span><span class="sxs-lookup"><span data-stu-id="778c6-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="778c6-128">Vad ska jag förvänta mig?</span><span class="sxs-lookup"><span data-stu-id="778c6-128">What should I expect?</span></span>
<span data-ttu-id="778c6-129">Som standard kan du använda exempelprogrammet för att ange en ordlista med kodmappningar för affärsenhet till företag.</span><span class="sxs-lookup"><span data-stu-id="778c6-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="778c6-130">Alla entiteter du initierar med fältet **OwningBusinessUnitOwningBusinessUnit** ställs automatiskt in på att använda det angivna företaget.</span><span class="sxs-lookup"><span data-stu-id="778c6-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="778c6-131">Alla entiteter utan fältet **OwningBusinessUnit**, t.ex. produkt, ställer in företaget baserat på mappningen med ett tomt affärsenhetsvärde.</span><span class="sxs-lookup"><span data-stu-id="778c6-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="778c6-132">Konsolprogrammet förväntar sig en parameter, antingen **–simulera** eller **–tillämpa**.</span><span class="sxs-lookup"><span data-stu-id="778c6-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="778c6-133">Om du använder kommandoradparametern **–simulera** uppdateras inga data.</span><span class="sxs-lookup"><span data-stu-id="778c6-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="778c6-134">Endast **simulation_<entityname>.csv**-filer genereras i samma katalog som verktyget, en för varje entitet som skulle ha uppdaterats.</span><span class="sxs-lookup"><span data-stu-id="778c6-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="778c6-135">Du kan granska dessa filer iterativt medan du arbetar för att säkerställa att koden uppdaterar företagsvärden som förväntat.</span><span class="sxs-lookup"><span data-stu-id="778c6-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="778c6-136">När du är klar med de simulerade uppdateringarna använder du parametern **–tillämpa**.</span><span class="sxs-lookup"><span data-stu-id="778c6-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="778c6-137">Detta uppdaterar alla rader som för närvarande har ett felaktigt företagsvärde i grupper om 1000 rader i taget (som standard).</span><span class="sxs-lookup"><span data-stu-id="778c6-137">This updates all rows that currently have an incorrect company value, in batches of 1000 rows at a time (by default).</span></span> <span data-ttu-id="778c6-138">Koden är idempotent enligt det som anges, vilket innebär att du kan köra den igen och att endast de felaktigt tilldelade företagen uppdateras.</span><span class="sxs-lookup"><span data-stu-id="778c6-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="778c6-139">När du kör med **–tillämpa** används kodutgångar CSV-filer av de ändringar som har gjorts, med namnet **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="778c6-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
