...
# <a name="cost-accounting-terminology"></a>Terminologi i kostnadsredovisning
...
Kostnadselement används som en funktion för att följa och kategorisera var kostnader flödar till. Det finns två typer av kostnadselement: primära kostnader och sekundära kostnader. **Primärkostnader** primära kostnadselement som representerar flödet av kostnader från Affärsredovisning till kostnadsredovisningen. Kostnadselementstrukturen motsvarar resultaträkningsstrukturen i huvudboken, där ett kostnadselement kan motsvara en huvudkonto. Alla rubrikkontona måste representeras som kostnadselement beroende på företagets behov. Nedan följer några exempel på primära kostnadselement:
...
- **Fast kostnad** -fast kostnad är en kostnad som inte skiljer sig åt på kort sikt, oavsett ändringar i aktivitetsnivån. Exempelvis kan en fast kostnad vara en grundläggande rörelseutgift i ett företag som till exempel hyra, som inte kommer att påverkas även om aktivitetsnivån ökar eller minskar.

- **Variabel kostnad** -en variabel kostnad ändras i enlighet med ändringar i aktivitetsnivån. Till exempel är specifika direktmaterialkostnader kopplade till varje produkt som säljs. Ju fler produkter som säljs, desto fler direkta materialkostnader finns det.

- **Delvis variabla kostnaderna** -delvis Rörliga kostander är kostnader för delvis fasta och variabla delvis. Till exempel en avgift för Internetåtkomst inkluderar en månatlig standardåtkomstavgift och en användningsavgift för bredband. Den månatliga standardåtkomstavgiften är en fast kostnad, medan användningavgiften för bredband är en variabel kostnad.
